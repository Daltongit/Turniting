// =========================================================================
// 1. INICIALIZACIÓN DE SUPABASE (Global)
// =========================================================================
window.miSupabase = null;
try {
    if (typeof supabase !== 'undefined') {
        window.miSupabase = supabase.createClient('https://osriruqcnxshmkvdhijw.supabase.co', 'sb_publishable_9-dt8ZHtX3uAQtb4vMPKGQ__34i08qS');
    }
} catch(err) { console.warn("Supabase cargará en offline mode."); }


// =========================================================================
// 2. FUNCIONES GLOBALES DE PRESENTACIÓN (12 Diapositivas)
// =========================================================================
window.currentSlide = 0;
window.totalSlides = 12; // AHORA SON 12 DIAPOSITIVAS
...
window.actualizarUI = function() {
    ...
    if (counter) counter.innerText = (window.currentSlide + 1) + " / " + window.totalSlides;
    ...
    if (btnNext) btnNext.style.opacity = window.currentSlide === window.totalSlides - 1 ? "0.3" : "1";
};
...
window.iniciarSimuladorAvanzado = async function() {
    ...
    chatHistory.innerHTML = ""; thoughtProcess.innerHTML = "";
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        if(step.msg) {
            ...
            real.className = `msg-bubble ${step.msg.r}`; real.textContent = step.msg.t;
...
// =========================================================================
// 3. FUNCIONES GLOBALES DEL TEST (BLINDADAS A EVENTOS ONCLICK)
// =========================================================================
...
window.currentPlayerAvatar = "fa-robot";
...
window.limpiarSesionSupabase = async function() {
    ...
    try {
        if(window.roomSubscription) window.miSupabase.removeChannel(window.roomSubscription);
        if(window.playersSubscription) window.miSupabase.removeChannel(window.playersSubscription);
        if(window.isHost && window.currentRoomCode) await window.miSupabase.from('rooms').update({ state: 'finished' }).eq('code', window.currentRoomCode);
    } catch(e) {}
    window.currentRoomCode = ""; window.myPlayerId = ""; window.isHost = false; clearInterval(window.timerInterval);
...
// CREAR SALA (HOST)
window.crearSalaHost = async function() {
    if(!window.miSupabase) { alert("Conectando a la base de datos, espera un segundo..."); return; }
    window.isHost = true;
    window.currentRoomCode = Math.random().toString(36).substring(2, 6).toUpperCase();
    
    try {
        const { error } = await window.miSupabase.from('rooms').insert([{ code: window.currentRoomCode, state: 'lobby' }]);
        ...
        window.playersSubscription = window.miSupabase.channel('host_players')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'players', filter: `room_code=eq.${window.currentRoomCode}` }, async () => {
                const { data, error } = await window.miSupabase.from('players').select('*').eq('room_code', window.currentRoomCode);
                if(error) return;
                ...
                const { data: roomData } = await window.miSupabase.from('rooms').select('state').eq('code', window.currentRoomCode).single();
                if(roomData && (roomData.state === 'playing' || roomData.state === 'finished')) window.pintarTablaPosiciones(data);
            }).subscribe();
    } catch(err) { console.error(err); alert("Error crítico al crear sala."); }
};

// EMPEZAR PARTIDA (HOST)
window.empezarPartidaHost = async function() {
    if(!window.miSupabase) return;
    await window.miSupabase.from('rooms').update({ state: 'playing' }).eq('code', window.currentRoomCode);
    const { data } = await window.miSupabase.from('players').select('*').eq('room_code', window.currentRoomCode);
    window.pintarTablaPosiciones(data); 
    window.mostrarPantallaJuego('leaderboard-screen');
};

// SELECCIONAR AVATAR
window.seleccionarAvatar = function(element) {
    document.querySelectorAll('.avatar-option').forEach(o => o.classList.remove('active')); 
    element.classList.add('active');
    window.currentPlayerAvatar = element.getAttribute('data-avatar');
};

// UNIRSE A SALA (JUGADOR)
window.unirseSalaJugador = async function() {
    if(!window.miSupabase) { alert("Conectando a la base de datos, espera..."); return; }
    ...
    if(!c || c.length !== 4) { alert("Código inválido de 4 letras."); return; }
    
    try {
        const { data: room, error: roomErr } = await window.miSupabase.from('rooms').select('*').eq('code', c).single();
        if(roomErr || !room) { alert("La sala no existe."); return; }
        
        window.currentRoomCode = c;
        const { data: player, error: pErr } = await window.miSupabase.from('players').insert([{ room_code: c, name: n, avatar: window.currentPlayerAvatar }]).select().single();
        ...
        document.getElementById('my-waiting-avatar').className = `fa-solid ${window.currentPlayerAvatar}`;
        window.mostrarPantallaJuego('waiting-screen');
        
        window.roomSubscription = window.miSupabase.channel('player_room_listen')
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'rooms', filter: `code=eq.${window.currentRoomCode}` }, payload => {
                if(payload.new.state === 'playing') window.iniciarTestJugador(); 
            }).subscribe();
            
        if(room.state === 'playing') window.iniciarTestJugador();
    } catch(err) { console.error(err); alert("Error al unirse."); }
};


// FLUJO DE PREGUNTAS
window.iniciarTestJugador = function() {
    let shuffled = [...questionPool].sort(() => 0.5 - Math.random());
    window.gameQuestions = shuffled.slice(0, 5); window.currentQIndex = 0; window.score = 0;
    window.mostrarPantallaJuego('game-screen'); window.cargarPregunta();
};

window.cargarPregunta = function() {
    clearInterval(window.timerInterval); window.timeLeft = 15; window.updateTimerUI();
    const qData = window.gameQuestions[window.currentQIndex];
    document.getElementById('current-q-num').innerText = window.currentQIndex + 1;
    ...
    qData.options.forEach((optText, i) => {
        ...
        btn.addEventListener('click', () => window.procesarRespuesta(i, btn)); container.appendChild(btn);
    });
    window.timerInterval = setInterval(() => {
        window.timeLeft--; window.updateTimerUI();
        if(window.timeLeft <= 0) { clearInterval(window.timerInterval); window.procesarRespuesta(-1, null); }
    }, 1000);
};

window.updateTimerUI = function() {
    document.getElementById('timer-count').innerText = window.timeLeft;
    document.getElementById('timer-bar-fill').style.width = `${(window.timeLeft / 15) * 100}%`;
};

window.procesarRespuesta = async function(selectedIndex, btnElement) {
    clearInterval(window.timerInterval); 
    const correctIndex = window.gameQuestions[window.currentQIndex].a;
    const btns = document.querySelectorAll('.option-btn'); btns.forEach(b => b.disabled = true);
    if(selectedIndex === correctIndex) { if(btnElement) btnElement.classList.add('correct'); window.score += 100 + (window.timeLeft * 10); 
    } else { if(btnElement) btnElement.classList.add('wrong'); btns[correctIndex].classList.add('correct'); }
    if(window.miSupabase) await window.miSupabase.from('players').update({ score: window.score }).eq('id', window.myPlayerId);
    setTimeout(() => {
        window.currentQIndex++;
        if(window.currentQIndex < window.gameQuestions.length) window.cargarPregunta(); else window.terminarTestJugador();
    }, 2000);
};

window.terminarTestJugador = async function() {
    if(!window.miSupabase) return;
    const { data } = await window.miSupabase.from('players').select('*').eq('room_code', window.currentRoomCode);
    window.pintarTablaPosiciones(data); window.mostrarPantallaJuego('leaderboard-screen');
    if(!window.playersSubscription) {
         window.playersSubscription = window.miSupabase.channel('player_leaderboard')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'players', filter: `room_code=eq.${window.currentRoomCode}` }, async () => {
            const { data: newData } = await window.miSupabase.from('players').select('*').eq('room_code', window.currentRoomCode);
            window.pintarTablaPosiciones(newData);
        }).subscribe();
    }
};

window.pintarTablaPosiciones = function(playersData) {
    ...
    playersData.forEach((e, i) => {
        ...
        if(e.id === window.myPlayerId) tr.style.background = "rgba(0, 229, 255, 0.2)";
...
// =========================================================================
// 4. AL CARGAR LA PÁGINA: DIBUJAR TODO
// =========================================================================
