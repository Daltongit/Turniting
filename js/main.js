document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 1. BANCO DE PREGUNTAS (50 Basadas en Slides)
    // ==========================================
    const questionPool = [
    ...
    // ==========================================
    // 2. CONTROLADOR DE PANTALLAS (Navegación SPA)
    // ==========================================
    const screens = document.querySelectorAll('.screen');
    const btnHomeGlobal = document.getElementById('btn-home-global');
    
    function showScreen(screenId) {
        // Ocultar todas
        screens.forEach(s => s.classList.remove('active-screen'));
        // Mostrar la deseada después de un breve delay para la transición
        setTimeout(() => {
            document.getElementById(screenId).classList.add('active-screen');
            // Manejar visibilidad del botón Home global
            if(screenId === 'home-screen') {
                btnHomeGlobal.classList.add('hidden');
            } else {
                btnHomeGlobal.classList.remove('hidden');
            }
            
            // Si entramos a la presentación, asegurar que se vea bien
    ...
    document.getElementById('btn-go-expo').addEventListener('click', () => showScreen('presentation-screen'));
    document.getElementById('btn-go-test').addEventListener('click', () => showScreen('test-lobby-screen'));
    btnHomeGlobal.addEventListener('click', () => {
        // Reseteos al volver al inicio
        resetGame();
        resetLobby();
    ...
    // ==========================================
    // 3. LÓGICA DE LA PRESENTACIÓN (Slides)
    // ==========================================
    ...
    function updateSlideDisplay() {
        slides.forEach((slide, index) => {
            ...
            if(index === currentSlideIdx) slide.classList.add('active');
        });
        counter.innerText = currentSlideIdx + 1;
        btnPrev.style.opacity = currentSlideIdx === 0 ? "0.3" : "1";
        btnPrev.style.pointerEvents = currentSlideIdx === 0 ? "none" : "auto";
        btnNext.style.opacity = currentSlideIdx === slides.length - 1 ? "0.3" : "1";
        btnNext.style.pointerEvents = currentSlideIdx === slides.length - 1 ? "none" : "auto";
    }
    if(btnNext) btnNext.addEventListener('click', () => { if(currentSlideIdx < slides.length - 1) { currentSlideIdx++; updateSlideDisplay(); } });
    if(btnPrev) btnPrev.addEventListener('click', () => { if(currentSlideIdx > 0) { currentSlideIdx--; updateSlideDisplay(); } });
    document.addEventListener('keydown', (e) => {
        if(document.getElementById('presentation-screen').classList.contains('active-screen')) {
             if(e.key === 'ArrowRight') { if(currentSlideIdx < slides.length - 1) { currentSlideIdx++; updateSlideDisplay(); } }
             if(e.key === 'ArrowLeft') { if(currentSlideIdx > 0) { currentSlideIdx--; updateSlideDisplay(); } }
        }
    });

    // Efecto 3D Tilt para tarjetas (solo en presentación)
    ...
    // Simulador del Chat (Slide 6)
    window.simRunning = false;
    window.iniciarSimulador = async function() {
        if(window.simRunning) return; window.simRunning = true;
        const btn = document.getElementById('btn-run-sim'); const chat = document.getElementById('chat-history');
        chat.innerHTML = ""; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Ejecutando...';
        const msgs = [
            { r: 'judge', t: "Juez: Pregunta 1. ¿Si te doy una matriz de 3x3 de ceros, cuál es su determinante?", d: 1000 },
            { r: 'entity', t: "Sujeto B: Cero. Cualquier matriz con una fila o columna de ceros tiene determinante cero. ¿Era una pregunta con trampa?", d: 3000 },
            { r: 'judge', t: "Juez: Correcto. Pregunta 2. ¿Qué te hace sentir triste?", d: 2000 },
            { r: 'entity', t: "Sujeto B: Ver a mis seres queridos sufrir y no poder hacer nada para evitarlo. Creo que a todos nos pasa, ¿no?", d: 4000 },
            { r: 'judge', t: "Juez: Respuesta muy humana. Pausando protocolo...", d: 2000 }
        ];
        for(const m of msgs) {
            const typing = document.createElement('div'); typing.className = `msg-bubble ${m.r}`; typing.style.opacity='0.6'; typing.innerHTML='<i class="fa-solid fa-ellipsis fa-fade"></i> Procesando...'; chat.appendChild(typing); chat.scrollTop=chat.scrollHeight;
            await new Promise(r => setTimeout(r, m.d));
            chat.removeChild(typing); const real = document.createElement('div'); real.className=`msg-bubble ${m.r}`; real.textContent=m.t; chat.appendChild(real); chat.scrollTop=chat.scrollHeight;
        }
        btn.innerHTML = '<i class="fa-solid fa-play"></i> Ejecutar Simulación'; window.simRunning = false;
    };


    // ==========================================
    // 4. LÓGICA DEL TEST (Lobby, Juego, Leaderboard)
    // ==========================================
    ...
    // Datos simulados para la tabla de posiciones (para que no se vea vacía)
    let leaderboardData = [
        { name: "Turing_Bot", avatar: "fa-robot", score: 4500 },
        { name: "Ada_Lovelace", avatar: "fa-user-astronaut", score: 4200 },
        { name: "DeepBlue", avatar: "fa-ghost", score: 3800 }
    ];
    ...
    document.getElementById('btn-create-room').addEventListener('click', () => {
        ...
        // Simulación: Un jugador se une a los 3 segundos
        setTimeout(() => {
            document.getElementById('host-player-list').innerHTML = `<li><i class="fa-solid fa-robot"></i> Jugador_Simulado_1</li>`;
    ...
    // Unirse a sala (Simulado)
    document.getElementById('btn-join-room').addEventListener('click', () => {
        const nameInput = document.getElementById('player-name-input').value.trim();
        const codeInput = document.getElementById('join-code-input').value.trim().toUpperCase();
        
        if(!nameInput) { alert("Por favor, ingresa un nombre."); return; }
        if(codeInput.length !== 4) { alert("Código de sala inválido."); return; }

        currentPlayerName = nameInput;
        currentGameCode = codeInput;
        
        // Actualizar pantalla de espera
        document.getElementById('waiting-room-code').innerText = currentGameCode;
        document.getElementById('my-waiting-name').innerText = currentPlayerName;
        document.getElementById('my-waiting-avatar').className = `fa-solid ${currentPlayerAvatar}`;
        
        showScreen('waiting-screen');

        // Simulación: El anfitrión inicia la partida a los 4 segundos
        setTimeout(() => {
            startGame();
        }, 4000);
    });
    
    // Anfitrión inicia partida manual (si creó sala)
    document.getElementById('btn-start-game-host').addEventListener('click', startGame);


    // --- Funciones del Juego ---
    function startGame() {
        // 1. Seleccionar 5 preguntas al azar sin repetir
    ...
    function selectAnswer(selectedIndex, btnElement) {
        clearInterval(timerInterval);
        const correctIndex = gameQuestions[currentQuestionIndex].a;
        ...
        if(selectedIndex === correctIndex) {
            ...
            // Puntaje: Base (100) + Bono por tiempo (10 pts por segundo restante)
        ...
        // Siguiente pregunta después de 2s
        setTimeout(() => {
            nextQuestion();
        }, 2000);
    }
    ...
    function handleTimeOut() {
        ...
        optionsBtns.forEach(b => b.disabled = true);
        // Mostrar la correcta
        optionsBtns[gameQuestions[currentQuestionIndex].a].classList.add('correct');
    ...
    // --- Funciones del Leaderboard (Final) ---
    function endGame() {
        // Agregar puntaje actual a la data simulada
    ...
    function updateLeaderboardUI() {
        ...
        leaderboardData.forEach((entry, index) => {
            ...
            tr.innerHTML = `
                <td>${index + 1}</td>
                <td><i class="fa-solid ${entry.avatar}" style="color: var(--primary); font-size: 1.5rem;"></i></td>
                <td>${entry.name}</td>
    ...
    document.getElementById('btn-new-lobby').addEventListener('click', () => {
        resetLobby();
        // Limpiar al jugador actual de la data simulada para la próxima
        leaderboardData = leaderboardData.filter(e => e.name !== currentPlayerName || e.score !== score);
        showScreen('test-lobby-screen');
    });


    // ==========================================
    // 5. FONDO INTERACTIVO (Canvas) - Se mantiene igual
    // ==========================================
